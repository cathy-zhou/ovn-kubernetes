...
func (bnc *BaseNetworkController) cleanupPodSelectorAddressSets() error {
	...
	return deleteAddrSetsWithoutACLRef(asPred, bnc.nbClient)
}
...
func (bnc *BaseNetworkController) syncAddressSets() error {
	...
	addrSetSyncer := addrsetsyncer.NewAddressSetSyncer(bnc.nbClient, bnc.controllerName)
	err := addrSetSyncer.SyncAddressSets()
	if err != nil {
		return fmt.Errorf("failed to sync address sets on controller init: %v", err)
	}

	existingNodes, err := bnc.kube.GetNodes()
	if err != nil {
		return fmt.Errorf("failed to get existing nodes: %w", err)
	}
	aclSyncer := aclsyncer.NewACLSyncer(bnc.nbClient, bnc.controllerName)
