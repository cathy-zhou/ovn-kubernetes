...
func (oc *Controller) initSubnetAllocator(netName string) error {
	...
	for _, clusterEntry := range clusterIPNet {
		err := masterSubnetAllocator.AddNetworkRange(clusterEntry.CIDR.String(), clusterEntry.HostBits())
		if err != nil {
			return err
		}
	}
	...
	oc.netAttchmtDefs[netName].masterSubnetAllocator = masterSubnetAllocator
	return nil
}
...
func (oc *Controller) SetupMaster(netConf *ovntypes.NetConf) (*networkAttachmentDefinitionConfig, error) {
	...
	cmdArgs = []string{"--", "--may-exist", "ls-add", netPrefix + "join",
		"--", "set", "logical_switch", netPrefix + "join", fmt.Sprintf("other-config:subnet=%s", joinCIDR.String()),
		"--", "set", "logical_switch", netPrefix + "join", fmt.Sprintf("other-config:exclude_ips=%s", joinIP.String())}
	if netName != "" {
		cmdArgs = append(cmdArgs, "external_ids:network_name="+netName)
	}
	...
	stdout, stderr, err = util.RunOVNNbctl(cmdArgs...)
	if err != nil {
		logrus.Errorf("Failed to create logical switch called \""+netPrefix+"join"+"\", stdout: %q, stderr: %q, error: %v", stdout, stderr, err)
		return nil, err
	}
	...
	stdout, stderr, err = util.RunOVNNbctl("--", "--may-exist", "lsp-add", netPrefix+"join", "jtor-"+netPrefix+OvnClusterRouter,
	...
	if err != nil {
		logrus.Errorf("Failed to add router-type logical switch port to "+netPrefix+"join"+", stdout: %q, stderr: %q, error: %v",
			stdout, stderr, err)
		return nil, err
	}
...
func (oc *Controller) deleteMaster(netName string) {
	...
	// The "join" switch will be allocated IP addresses in the range 100.64.0.0/16.
	stdout, stderr, err := util.RunOVNNbctl("--if-exist", "ls-del", netPrefix+"join")
	if err != nil {
		logrus.Errorf("Failed to delete logical switch called \""+netPrefix+"join"+"\", stdout: %q, stderr: %q, error: %v", stdout, stderr, err)
	}
...
func (oc *Controller) addNode(node *kapi.Node) (map[string]*net.IPNet, error) {
	...
	for netName := range oc.netAttchmtDefs {
		...
		// not release the allocated subnets as they are already annotated on the node
		if err != nil {
			oc.netMutex.Unlock()
			oc.nodeMutex.Unlock()
			return nil, fmt.Errorf("failed to allocate subnet for default network on node %s: %v", node.Name, err)
		}
...
func (oc *Controller) getHostSubnet(nodeName, netName string, create bool) (hostsubnet *net.IPNet, err error) {
	...
	hostsubnetStr, err := oc.netAttchmtDefs[netName].masterSubnetAllocator.AllocateNetwork()
	if err != nil {
	...
	netPrefix := util.GetNetworkPrefix(netName)
	err = oc.kube.SetAnnotationOnNode(nodeName, netPrefix+OvnHostSubnet, hostsubnetStr)
	if err != nil {
		logrus.Errorf("Failed to set node %s host subnet annotation to %q: %v",
			nodeName, hostsubnetStr, err)
		return nil, err
	}

	logrus.Debugf("getHostSubnet successfully allocated subnet %s for node %s NetName %s", hostsubnetStr, nodeName, netName)
	oc.nodeCache[nodeName][netName] = hostsubnetStr
...
func (oc *Controller) deleteNodeHostSubnet(nodeName string, subnet *net.IPNet, netName string) error {
	...
	err := oc.netAttchmtDefs[netName].masterSubnetAllocator.ReleaseNetwork(subnet.String())
	if err != nil {
		return fmt.Errorf("Error deleting subnet %v for node %q network %s: %s", subnet, nodeName, netName, err)
	}
	logrus.Infof("Deleted HostSubnet %v for node %s network %s", subnet, nodeName, netName)
	return nil
}
...
func (oc *Controller) deleteNode(nodeName string) error {
	...
	deleteNetworks := make(map[string]*net.IPNet)
	oc.nodeMutex.Lock()
	oc.netMutex.Lock()
	for netName := range oc.netAttchmtDefs {
		...
		oc.lsMutex.Unlock()
		deleteNetworks[netName] = subnet
	}
	delete(oc.nodeCache, nodeName)
	oc.netMutex.Unlock()
	oc.nodeMutex.Unlock()
	for netName, nodeSubnet := range deleteNetworks {
