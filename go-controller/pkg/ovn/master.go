...
func (oc *Controller) addNode(node *kapi.Node) ([]*net.IPNet, error) {
	...
	if err != nil {
		...
		klog.V(5).Infof("Node %s is added with new chassis ID, delete its stale chassis in SBDB", node.Name)
		if err = oc.deleteNodeChassis(node.Name); err != nil {
			oc.recorder.Eventf(node, kapi.EventTypeWarning, "ErrorMismatchChassis",
				"Node %s is added with new chassis ID. Its stale chassis entry is still in the SBDB",
				node.Name)
		}
	}
