	...
	lec := leaderelection.LeaderElectionConfig{
		...
		Callbacks: leaderelection.LeaderCallbacks{
			OnStartedLeading: func(ctx context.Context) {
				...
				if err := mc.ovnController.Run(mc.wg); err != nil {
					panic(err.Error())
				}
			},
...
func (oc *Controller) deleteMaster() {
	...
	for _, node := range existingNodes.Items {
		_ = oc.setNodeAnnotations(&node, []*net.IPNet{})
	}
}
...
func (oc *Controller) ensureNodeLogicalNetwork(nodeName string, hostSubnets []*net.IPNet) error {
	...
	if !oc.netconf.NotDefault {
		if err = addToPortGroup(clusterRtrPortGroupName, &lpInfo{
			...
			//klog.Errorf(err.Error())
			klog.Errorf("add port %s to portgroup %s failed not_default: %v netname: %s err: %v", types.SwitchToRouterPrefix + switchName,
				clusterRtrPortGroupName, oc.netconf.NotDefault, oc.netconf.Name, err)
			return err
		}
...
func (oc *Controller) setNodeAnnotations(node *kapi.Node, hostSubnets []*net.IPNet) error {
	var nodeAnnotations map[string]interface{}
	var err error

	if len(hostSubnets) == 0 {
		nodeAnnotations = map[string]interface{}{
			util.GetAnnotationName(util.OvnNodeSubnets, oc.netconf.Name): nil,
		}
	} else {
		nodeAnnotations, err = util.CreateNodeHostSubnetAnnotation(hostSubnets, oc.netconf.Name)
		if err != nil {
...
func (oc *Controller) addNode(node *kapi.Node) ([]*net.IPNet, error) {
	...
	err = oc.setNodeAnnotations(node, hostSubnets)
	if err != nil {
		return nil, err
	}
