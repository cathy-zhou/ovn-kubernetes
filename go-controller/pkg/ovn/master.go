...
func (nci *NetworkControllerInfo) createOvnClusterRouter(multicastSupport bool) (*nbdb.LogicalRouter, error) {
	...
	logicalRouterName := nci.GetPrefix() + types.OVNClusterRouter
...
func (oc *DefaultNetworkController) SetupMaster(existingNodeNames []string) error {
	// Create default Control Plane Protection (COPP) entry for routers
	logicalRouter, err := oc.createOvnClusterRouter(oc.multicastSupport)
	if err != nil {
		return err
	}
...
func (nci *NetworkControllerInfo) syncNodeClusterRouterPort(node *kapi.Node, hostSubnets []*net.IPNet) error {
	...
	if len(hostSubnets) == 0 {
		hostSubnets, err = util.ParseNodeHostSubnetAnnotation(node, nci.GetNetworkName())
		if err != nil {
			return err
		}
	}
	...
	switchName := nci.GetPrefix() + node.Name
	logicalRouterName := nci.GetPrefix() + types.OVNClusterRouter
...
func (nci *NetworkControllerInfo) createNodeLogicalSwitch(nodeName string, hostSubnets []*net.IPNet,
	...
	// logical router port MAC is based on IPv4 subnet if there is one, else IPv6
	var nodeLRPMAC net.HardwareAddr
	switchName := nci.GetPrefix() + nodeName
	for _, hostSubnet := range hostSubnets {
		gwIfAddr := util.GetNodeGatewayIfAddr(hostSubnet)
		nodeLRPMAC = util.IPAddrToHWAddr(gwIfAddr.IP)
		if !utilnet.IsIPv6CIDR(hostSubnet) {
...
func (oc *DefaultNetworkController) ensureNodeLogicalNetwork(node *kapi.Node, hostSubnets []*net.IPNet) error {
	logicalSwitch, err := oc.createNodeLogicalSwitch(node.Name, hostSubnets,
...
func (nci *NetworkControllerInfo) createNodeSubnetAnnotation(node *kapi.Node,
	...
	if err != nil && !util.IsAnnotationNotSetError(err) {
		...
		klog.Infof("Failed to get node %s host subnets annotations for network %s: %v", node.Name, nci.GetNetworkName(), err)
	}
...
func (nci *NetworkControllerInfo) updateFoundNodes(node *kapi.Node,
	...
	hostSubnets, _ := util.ParseNodeHostSubnetAnnotation(node, nci.GetNetworkName())
	foundNodes.Insert(node.Name)

	klog.V(5).Infof("Node %s contains subnets: %v network %s", node.Name, hostSubnets, nci.GetNetworkName())
