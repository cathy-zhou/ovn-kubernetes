...
func (oc *DefaultL3Controller) upgradeToNamespacedDenyPGOVNTopology(existingNodeList *kapi.NodeList) error {
...
func (oc *DefaultL3Controller) upgradeToSingleSwitchOVNTopology(existingNodeList *kapi.NodeList) error {
	...
	for _, legacyJoinSwitch := range legacyJoinSwitches {
		...
		if _, ok := existingNodes[nodeName]; !ok {
			_ = oc.controllerInfo.deleteNodeLogicalNetwork(nodeName)
			upgradeOnly = false
		}
...
func (oc *DefaultL3Controller) upgradeOVNTopology(existingNodes *kapi.NodeList) error {
...
func (oc *DefaultL3Controller) StartClusterMaster() error {
...
func (cInfo *controllerInfo) createOvnClusterRouter(multicastSupport bool) (*nbdb.LogicalRouter, error) {
	// Create default Control Plane Protection (COPP) entry for routers
	defaultCOPPUUID, err := EnsureDefaultCOPP(cInfo.nbClient)
	if err != nil {
		return nil, fmt.Errorf("unable to create router control plane protection: %w", err)
	}

	// Create a single common distributed router for the cluster.
	logicalRouterName := cInfo.Prefix + types.OVNClusterRouter
	...
	if cInfo.IsSecondary {
		logicalRouter.ExternalIDs[types.NetworkNameExternalID] = cInfo.NetName
	}
	if multicastSupport {
	...
	err = libovsdbops.CreateOrUpdateLogicalRouter(cInfo.nbClient, &logicalRouter)
	if err != nil {
...
func (oc *DefaultL3Controller) SetupMaster(existingNodeNames []string) error {
	// Create default Control Plane Protection (COPP) entry for routers
	logicalRouter, err := oc.controllerInfo.createOvnClusterRouter(oc.multicastSupport)
	if err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("failed to add logical router port %+v on router %+v: %v", logicalRouterPort, logicalRouter.Name, err)
	}
...
func (oc *DefaultL3Controller) syncNodeManagementPort(node *kapi.Node, hostSubnets []*net.IPNet) error {
...
func (oc *DefaultL3Controller) syncGatewayLogicalNetwork(node *kapi.Node, l3GatewayConfig *util.L3GatewayConfig,
...
// problems.
func (cInfo *controllerInfo) syncNodeClusterRouterPort(node *kapi.Node, hostSubnets []*net.IPNet) error {
	...
	if len(hostSubnets) == 0 {
		hostSubnets, err = util.ParseNodeHostSubnetAnnotation(node, cInfo.NetName)
		if err != nil {
			return err
		}
	}
	...
	switchName := cInfo.Prefix + node.Name
	logicalRouterName := cInfo.Prefix + types.OVNClusterRouter
	...
	err = libovsdbops.CreateOrUpdateLogicalRouterPorts(cInfo.nbClient, &logicalRouter,
		[]*nbdb.LogicalRouterPort{&logicalRouterPort}, &logicalRouterPort.MAC, &logicalRouterPort.Networks)
	if err != nil {
	...
	err = libovsdbops.CreateOrUpdateGatewayChassis(cInfo.nbClient, &logicalRouterPort, &gatewayChassis,
...
func (cInfo *controllerInfo) createNodeLogicalSwitch(nodeName string, hostSubnets []*net.IPNet,
	...
	if cInfo.IsSecondary {
		logicalSwitch.ExternalIDs = map[string]string{"network_name": cInfo.NetName}
	}
	...
	err = libovsdbops.CreateOrUpdateLogicalSwitch(cInfo.nbClient, &logicalSwitch, &logicalSwitch.OtherConfig,
	...
	err = libovsdbops.CreateOrUpdateLogicalSwitchPortsOnSwitch(cInfo.nbClient, &sw, &logicalSwitchPort)
	if err != nil {
	...
	if multicastSupport {
		err = libovsdbops.AddPortsToPortGroup(cInfo.nbClient, types.ClusterRtrPortGroupName, logicalSwitchPort.UUID)
		if err != nil {
			klog.Errorf(err.Error())
			return nil, err
		}
...
func (oc *DefaultL3Controller) ensureNodeLogicalNetwork(node *kapi.Node, hostSubnets []*net.IPNet) error {
	logicalSwitch, err := oc.controllerInfo.createNodeLogicalSwitch(node.Name, hostSubnets,
		oc.loadBalancerGroupUUID, oc.multicastSupport)
	if err != nil {
		return err
	}
	...
	lrpIPs, err := oc.joinSwIPManager.EnsureJoinLRPIPs(logicalSwitch.Name)
	if err != nil {
		return fmt.Errorf("failed to get join switch port IP address for switch %s: %v", logicalSwitch.Name, err)
	}
	...
	return oc.lsManager.AddNode(logicalSwitch.Name, logicalSwitch.UUID, hostSubnets)
}

func (cInfo *controllerInfo) createNodeSubnetAnnotation(nodeAnnotation map[string]string, node *kapi.Node,
	masterSubnetAllocator *subnetallocator.HostSubnetAllocator) (map[string]string, []*net.IPNet, error) {
	existingSubnets, err := util.ParseNodeHostSubnetAnnotation(node, cInfo.NetName)
	if err != nil && !util.IsAnnotationNotSetError(err) {
		// Log the error and try to allocate new subnets
		klog.Infof("Failed to get node %s host subnets annotations for network %s: %v", node.Name, cInfo.NetName, err)
	}
	...
	nodeAnnotation, err = util.UpdateNodeHostSubnetAnnotation(nodeAnnotation, hostSubnets, cInfo.NetName)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to update node %q annotation subnet %s for network %s",
			node.Name, util.JoinIPNets(hostSubnets, ","), cInfo.NetName)
	}

	return nodeAnnotation, hostSubnets, nil
}

func (cInfo *controllerInfo) updateNodeAnnotationWithRetry(nodeName string, updatedNodeAnnotation map[string]string) error {
	...
	resultErr := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		node, err := cInfo.watchFactory.GetNode(nodeName)
		if err != nil {
			return err
		}
		...
		for k, v := range updatedNodeAnnotation {
			cnode.Annotations[k] = v
		}
		return cInfo.kube.UpdateNode(cnode)
	})
	if resultErr != nil {
...
func (oc *DefaultL3Controller) addNode(node *kapi.Node) ([]*net.IPNet, error) {
	gwLRPIPs, err := oc.joinSwIPManager.EnsureJoinLRPIPs(node.Name)
	if err != nil {
	...
	updatedNodeAnnotation, hostSubnets, err := oc.controllerInfo.createNodeSubnetAnnotation(updatedNodeAnnotation, node, oc.masterSubnetAllocator)
	if err != nil {
		return nil, err
	}

	err = oc.controllerInfo.updateNodeAnnotationWithRetry(node.Name, updatedNodeAnnotation)
	if err != nil {
		return nil, err
	}
...
func (oc *DefaultL3Controller) checkNodeChassisMismatch(node *kapi.Node) (string, error) {
	chassisID, err := util.ParseNodeChassisIDAnnotation(node)
	if err != nil {
		return "", nil
	}
...
func (oc *DefaultL3Controller) deleteStaleNodeChassis(node *kapi.Node) error {
...
func (cInfo *controllerInfo) deleteNodeLogicalNetwork(nodeName string) error {
	switchName := cInfo.Prefix + nodeName
	// Remove switch to lb associations from the LBCache before removing the switch
	lbCache, err := ovnlb.GetLBCache(cInfo.nbClient)
	if err != nil {
	...
	err = libovsdbops.DeleteLogicalSwitch(cInfo.nbClient, switchName)
	if err != nil {
	...
	err = libovsdbops.DeleteLogicalRouterPorts(cInfo.nbClient, &logicalRouter, &logicalRouterPort)
	if err != nil {
...
func (oc *DefaultL3Controller) deleteNode(nodeName string) error {
	oc.masterSubnetAllocator.ReleaseAllNodeSubnets(nodeName)

	if err := oc.controllerInfo.deleteNodeLogicalNetwork(nodeName); err != nil {
...
func (oc *DefaultL3Controller) clearInitialNodeNetworkUnavailableCondition(origNode *kapi.Node) {
...
func (oc *DefaultL3Controller) syncNodesPeriodic() {
...
func (cInfo *controllerInfo) updateExpectedNodes(node *kapi.Node,
	masterSubnetAllocator *subnetallocator.HostSubnetAllocator, foundNodes sets.String) []*net.IPNet {
	if noHostSubnet(node) {
		return []*net.IPNet{}
	}
	hostSubnets, _ := util.ParseNodeHostSubnetAnnotation(node, cInfo.NetName)
	foundNodes.Insert(node.Name)

	klog.V(5).Infof("Node %s contains subnets: %v network %s", node.Name, hostSubnets, cInfo.NetName)
...
func (oc *DefaultL3Controller) syncNodes(nodes []interface{}) error {
	...
	for _, tmp := range nodes {
		...
		hostSubnets := oc.controllerInfo.updateExpectedNodes(node, oc.masterSubnetAllocator, foundNodes)
...
func (oc *DefaultL3Controller) addUpdateNodeEvent(node *kapi.Node, nSyncs *nodeSyncs) error {
	...
	if nSyncs.syncClusterRouterPort {
		if err = oc.controllerInfo.syncNodeClusterRouterPort(node, nil); err != nil {
			errs = append(errs, err)
			oc.nodeClusterRouterPortFailed.Store(node.Name, true)
		} else {
	...
	if _, gwFailed := oc.gatewaysFailed.Load(node.Name); !gwFailed || !config.Gateway.DisableSNATMultipleGWs {
		if nSyncs.syncNode || nSyncs.syncGw { // do this only if it is a new node add or a gateway sync happened
			errors := oc.controllerInfo.requestAddPodOnNode(node.Name, oc.retryPods)
...
func (cInfo *controllerInfo) requestAddPodOnNode(nodeName string, retryPods *ovnretry.RetryFramework) []error {
	errs := []error{}
	options := metav1.ListOptions{
		FieldSelector:   fields.OneTermEqualSelector("spec.nodeName", nodeName).String(),
		ResourceVersion: "0",
	}
	pods, err := cInfo.client.CoreV1().Pods(metav1.NamespaceAll).List(context.TODO(), options)
	if err != nil {
		errs = append(errs, err)
...
func (oc *DefaultL3Controller) recordNodeErrorEvent(node *kapi.Node, nodeErr error) {
...
func (oc *DefaultL3Controller) deleteNodeEvent(node *kapi.Node) error {
...
func (oc *DefaultL3Controller) createACLLoggingMeter() error {
	band := &nbdb.MeterBand{
