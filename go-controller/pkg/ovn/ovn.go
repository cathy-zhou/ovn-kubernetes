...
func (mc *OvnMHController) NewOvnController(netconf *cnitypes.NetConf,
...
func (oc *Controller) WatchNodes() {
	...
	oc.nodeHandler = oc.mc.watchFactory.AddNodeHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(old, new interface{}) {
			...
			if failed || oc.gatewayChanged(oldNode, node) || nodeSubnetChanged(oldNode, node, oc.netconf.Name) || hostAddressesChanged(oldNode, node) {
...
func (mc *OvnMHController) syncNetworkAttachDefinition(netattachdefs []interface{}) {
	...
	for _, result := range strings.Split(nodeSwitches, "\n") {
		...
		if netName == ovntypes.DefaultNetworkName || !strings.HasPrefix(items[0], netName) {
		...
		oc := &Controller{netconf: &cnitypes.NetConf{NetConf: ctypes.NetConf{Name: netName}}}
