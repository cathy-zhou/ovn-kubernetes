...
func (bnc *BaseNetworkController) syncMultiNetworkPolicies(multiPolicies []interface{}) error {
	klog.Infof("Sync all multi-network-policies for network %s", bnc.GetNetworkName())
	...
	for _, npInterface := range multiPolicies {
		...
		if !bnc.shouldApplyMultiPolicy(policy) {
			klog.V(5).Infof("[network (%s)] skipping syncing policy %s/%s",
				bnc.GetNetworkName(), policy.Namespace, policy.Name)
			continue
		}
...
func (bnc *BaseNetworkController) shouldApplyMultiPolicy(mpolicy *multinetworkpolicyapi.MultiNetworkPolicy) bool {
	...
	for _, networkName := range policyForNetworks {
		...
		if bnc.IsNadExist(util.GetNadName(networkNamespace, networkName)) {
			return true
		}
	}
	return false
...
func convertMultiNetPolicyToNetPolicy(mpolicy *multinetworkpolicyapi.MultiNetworkPolicy) *knet.NetworkPolicy {
