	...
	for _, ocInfo := range fakeOvn.secondaryControllers {
		...
		for _, pod := range pods {
			...
			for nad, portInfo := range podInfo.allportInfo {
				...
				lsp := &nbdb.LogicalSwitchPort{
					...
					ExternalIDs: map[string]string{
						...
						ovntypes.NetworkExternalID:  "network1",
			...
			data = append(data, &nbdb.LogicalSwitch{
				...
				ExternalIDs: map[string]string{ovntypes.NetworkExternalID: "network1"},
			})
		}
	}
	return data
...
var _ = ginkgo.Describe("OVN MultiNetworkPolicy Operations", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		logicalLayer2Switch := &nbdb.LogicalSwitch{
			Name:        "network1_" + ovntypes.OVNLayer2Switch,
			UUID:        "network1_" + ovntypes.OVNLayer2Switch + "_UUID",
			ExternalIDs: map[string]string{ovntypes.NetworkExternalID: "network1"},
		}
		...
		nad, err = newNetworkAttachmentDefinition(
			...
			ovncnitypes.NetConf{
				...
				NADName:  util.GetNADName(namespaceName1, "nad1"),
				Subnets:  "10.1.1.1/24",
			},
		)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.Context("during execution", func() {
		ginkgo.It("correctly creating an multinetworkPolicy with a peer namespace label", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				gressPolicyExpectedData := getPolicyData(policy, nil, []string{namespace2.Name},
					nil, "", false, false, netInfo)
				defaultDenyExpectedData := getDefaultDenyData(policy, nil, "", false, netInfo)
		...
		ginkgo.It("correctly creates and deletes network policy and multi network policy with the same policy", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				nPodTest.addNetwork("network1", util.GetNADName(nad.Namespace, nad.Name), "", "", "", "10.1.1.2", "0a:58:0a:01:01:02")
				...
				defaultDenyExpectedData1 := getDefaultDenyData(networkPolicy, []string{nPodTest.portUUID}, "", false, &util.DefaultNetInfo{})
				...
				ocInfo, ok := fakeOvn.secondaryControllers["network1"]
				gomega.Expect(ok).To(gomega.Equal(true))
				portInfo := nPodTest.getNetworkPortInfo("network1", util.GetNADName(nad.Namespace, nad.Name))
				gomega.Expect(portInfo).NotTo(gomega.Equal(nil))
				ocInfo.asf.ExpectAddressSetWithIPs(namespaceName1, []string{portInfo.podIP})

				gressPolicyExpectedData2 := getPolicyData(networkPolicy, []string{portInfo.portUUID}, nil,
					[]int32{portNum}, "", false, false, netInfo)
				defaultDenyExpectedData2 := getDefaultDenyData(networkPolicy, []string{portInfo.portUUID}, "", false, netInfo)
