...
func (oc *DefaultL3Controller) gatewayCleanup(nodeName string) error {
...
func (oc *DefaultL3Controller) delPbrAndNatRules(nodeName string, lrpTypes []string) {
...
func (oc *DefaultL3Controller) staticRouteCleanup(nextHops []net.IP) {
	ips := sets.String{}
...
// - if the LRP exists and has the len(nexthops) == 1: it removes
// the LRP completely
func (oc *DefaultL3Controller) policyRouteCleanup(nextHops []net.IP) {
	for _, nextHop := range nextHops {
		gwIP := nextHop.String()
...
func (oc *DefaultL3Controller) multiJoinSwitchGatewayCleanup(nodeName string, upgradeOnly bool) error {
	gatewayRouter := types.GWRouterPrefix + nodeName
...
func (oc *DefaultL3Controller) removeLRPolicies(nodeName string, priorities []string) {
	if len(priorities) == 0 {
...
func (oc *DefaultL3Controller) cleanupDGP(nodes *kapi.NodeList) error {
