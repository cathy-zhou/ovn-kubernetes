...
// See if this pod needs to plumb over this Contoller's network, and return NetworkSelectionElement if it exists.
//
...
func (oc *Controller) isNetworkOnPod(pod *kapi.Pod) (bool, *networkattachmentdefinitionapi.NetworkSelectionElement,
	...
	// and the Pod's k8s.v1.cni.cncf.io/networks
	networks, err := netattachdefutils.ParsePodNetworkAnnotation(pod)
	if err != nil {
		if _, ok := err.(*networkattachmentdefinitionapi.NoK8sNetworkError); !ok {
			// multus won't add this Pod if this fails, should never happen
			klog.Errorf("Failed to get networks for pod %s: %v", podDesc, err)
			return false, nil, nil
		}
		networks = []*networkattachmentdefinitionapi.NetworkSelectionElement{}
	}
	// for default network controller, if default network selection element exists but it is not as expected,
	// return false
	if !oc.netconf.NotDefault {
		if defaultNetwork == nil {
			return true, nil, networks
			...
			return false, nil, networks
		}
		return true, defaultNetwork, networks
	}
...
func (oc *Controller) syncPods(pods []interface{}) {
	...
	if oc.netconf.NotDefault {
		cmdArgs = append(cmdArgs, "external_ids:nework_name="+oc.netconf.Name)
	} else {
...
func (oc *Controller) addLogicalPort(pod *kapi.Pod) (err error) {
	...
	on, network, networks := oc.isNetworkOnPod(pod)
	if !on {
