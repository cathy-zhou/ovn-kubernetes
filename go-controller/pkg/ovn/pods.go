...
func (cInfo *controllerInfo) updateExpectedLogicalPorts(pod *kapi.Pod, lsManager *lsm.LogicalSwitchManager,
	annotations *util.PodAnnotation, nadKeyName string, expectedLogicalPorts map[string]bool) error {
	var err error
	switchName := cInfo.Prefix + pod.Spec.NodeName
	if util.PodScheduled(pod) && util.PodWantsNetwork(pod) && !util.PodCompleted(pod) {
		...
		if !cInfo.IsSecondary {
			logicalPort = util.GetSecondaryNetworkLogicalPortName(pod.Namespace, pod.Name, nadKeyName)
		}
...
func (oc *DefaultL3Controller) syncPods(pods []interface{}) error {
	...
	for _, podInterface := range pods {
		...
		err = oc.controllerInfo.updateExpectedLogicalPorts(pod, oc.lsManager, annotations,
	...
	return oc.controllerInfo.deleteStaleLogicalSwitchPorts(oc.lsManager, expectedLogicalPorts)
}

func (cInfo *controllerInfo) deleteStaleLogicalSwitchPorts(lsManager *lsm.LogicalSwitchManager,
	expectedLogicalPorts map[string]bool) error {
	// get all the nodes from the watchFactory
	nodes, err := cInfo.watchFactory.GetNodes()
	if err != nil {
	...
	for _, n := range nodes {
		// skip nodes that are not running ovnk (inferred from host subnets)
		switchName := cInfo.Prefix + n.Name
		if lsManager.IsNonHostSubnetSwitch(switchName) {
			continue
		}
		...
		ops, err = libovsdbops.DeleteLogicalSwitchPortsWithPredicateOps(cInfo.nbClient, ops, &sw, p)
		if err != nil {
			return fmt.Errorf("could not generate ops to delete stale ports from logical switch %s (%+v)", switchName, err)
		}
	}

	_, err = libovsdbops.TransactAndCheck(cInfo.nbClient, ops)
	if err != nil {
		return fmt.Errorf("could not remove stale logicalPorts from switches for network %s (%+v)", cInfo.NetName, err)
	}
	return nil
}
...
func (cInfo *controllerInfo) lookupPortUUIDAndSwitchName(logicalPort string) (portUUID string, logicalSwitch string, err error) {
	lsp := &nbdb.LogicalSwitchPort{Name: logicalPort}
	lsp, err = libovsdbops.GetLogicalSwitchPort(cInfo.nbClient, lsp)
	if err != nil {
		return "", "", err
	}
	...
	nodeSwitches, err := libovsdbops.FindLogicalSwitchesWithPredicate(cInfo.nbClient, p)
	if err != nil {
...
func (cInfo *controllerInfo) deleteLogicalPort(pod *kapi.Pod, portInfo *lpInfo, nadKeyName string,
	...
	podDesc := fmt.Sprintf("pod %s/%s nad %s", pod.Namespace, pod.Name, nadKeyName)
	logicalPort := util.GetLogicalPortName(pod.Namespace, pod.Name)
	if cInfo.IsSecondary {
		logicalPort = util.GetSecondaryNetworkLogicalPortName(pod.Namespace, pod.Name, nadKeyName)
	}
	if portInfo == nil {
		...
		annotation, err := util.UnmarshalPodAnnotation(pod.Annotations, nadKeyName)
		if err != nil {
		...
		portUUID, switchName, err = cInfo.lookupPortUUIDAndSwitchName(logicalPort)
		if err != nil {
	...
	if switchName != cInfo.Prefix+pod.Spec.NodeName {
		klog.Errorf("Deleting %s expecting switch name: %s, OVN DB has switch name %s for port uuid %s",
			podDesc, cInfo.Prefix+pod.Spec.NodeName, switchName, portUUID)
	}
	...
	if util.PodCompleted(pod) {
		if shouldRelease, err = lsManager.ConditionalIPRelease(switchName, podIfAddrs, func() (bool, error) {
			pods, err := cInfo.watchFactory.GetAllPods()
			if err != nil {
			...
			for _, p := range pods {
				...
				pAddrs, err := util.GetAllPodIPs(p, &cInfo.NetInfo)
				if err != nil {
					continue
				}
	...
	if shouldRelease {
		if ops, err = cInfo.deletePodFromNamespace(nsm, pod.Namespace,
	...
	ops, err = cInfo.delLSPOps(lsManager, logicalPort, switchName, portUUID)
	...
	if !cInfo.IsSecondary {
		...
		recordOps, txOkCallBack, _, err = metrics.GetConfigDurationRecorder().AddOVN(cInfo.nbClient, "pod", pod.Namespace,
			pod.Name)
		if err != nil {
	...
	_, err = libovsdbops.TransactAndCheck(cInfo.nbClient, allOps)
	if err != nil {
...
func (oc *DefaultL3Controller) deleteLogicalPort(pod *kapi.Pod, portInfo *lpInfo) (err error) {
	...
	pInfo, err := oc.controllerInfo.deleteLogicalPort(pod, portInfo, ovntypes.DefaultNetworkName,
...
func (cInfo *controllerInfo) addRoutesGatewayIP(pod *kapi.Pod, network *networkattachmentdefinitionapi.NetworkSelectionElement,
...
func (oc *DefaultL3Controller) podExpectedInLogicalCache(pod *kapi.Pod) bool {
	return util.PodWantsNetwork(pod) && !oc.lsManager.IsNonHostSubnetSwitch(pod.Spec.NodeName) && !util.PodCompleted(pod)
}

func (cInfo *controllerInfo) addLogicalPort(pod *kapi.Pod, lsManager *lsm.LogicalSwitchManager,
	clusterSubnets []config.CIDRNetworkEntry, nadKeyName string,
	...
	podDesc := fmt.Sprintf("%s/%s/%s", nadKeyName, pod.Namespace, pod.Name)
	switchName := cInfo.Prefix + pod.Spec.NodeName
	...
	if cInfo.IsSecondary {
		portName = util.GetSecondaryNetworkLogicalPortName(pod.Namespace, pod.Name, nadKeyName)
	}
	...
	existingLSP, err := libovsdbops.GetLogicalSwitchPort(cInfo.nbClient, lsp)
	if err != nil && err != libovsdbclient.ErrNotFound {
		return nil, nil, nil, fmt.Errorf("unable to get the lsp %s from the nbdb: %s", portName, err)
	}
	...
	if lspExist {
		portFound := false
		ls, err = libovsdbops.GetLogicalSwitch(cInfo.nbClient, ls)
		if err != nil {
			return nil, nil, nil, fmt.Errorf("[%s] unable to find logical switch %s in NBDB",
				podDesc, switchName)
		}
		...
		if !portFound {
			...
			return nil, nil, nil, fmt.Errorf("[%s] failed to locate existing logical port %s (%s) in logical switch %s",
	...
	podAnnotation, err = util.UnmarshalPodAnnotation(pod.Annotations, nadKeyName)
	...
	if err == nil {
		...
		if err = lsManager.AllocateIPs(switchName, podIfAddrs); err != nil && err != ipallocator.ErrAllocated {
			return nil, nil, nil, fmt.Errorf("unable to ensure IPs allocated for already annotated pod: %s, IPs: %s, error: %v",
	...
	if needsIP {
		if existingLSP != nil {
			...
			if err != nil {
				return nil, nil, nil, fmt.Errorf("failed to get pod addresses for pod %s on node: %s, err: %v",
		...
		if needsNewAllocation {
			...
			if err != nil {
				return nil, nil, nil, fmt.Errorf("failed to assign pod addresses for pod %s on switch: %s, err: %v",
					podDesc, switchName, err)
			}
		}
		...
		if network != nil && network.MacRequest != "" {
			...
			if err != nil {
				return nil, nil, nil, fmt.Errorf("failed to parse mac %s requested in annotation for pod %s: Error %v",
		...
		if nodeSubnets = lsManager.GetSwitchSubnets(switchName); nodeSubnets == nil {
			return nil, nil, nil, fmt.Errorf("cannot retrieve subnet for assigning gateway routes for pod %s, switch: %s",
				podDesc, switchName)
		}
		err = cInfo.addRoutesGatewayIP(pod, network, clusterSubnets, podAnnotation, nodeSubnets)
		if err != nil {
			return nil, nil, nil, err
		}
		...
		annoStart := time.Now()
		err = cInfo.updatePodAnnotationWithRetry(pod, podAnnotation, nadKeyName)
	...
	if cInfo.IsSecondary {
		lsp.ExternalIDs[ovntypes.NetworkNameExternalID] = cInfo.NetName
	}

	// CNI depends on the flows from port security, delay setting it until end
	lsp.PortSecurity = addresses

	ops, err = libovsdbops.CreateOrUpdateLogicalSwitchPortsOnSwitchOps(cInfo.nbClient, nil, ls, lsp)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("error creating logical switch port %+v on switch %+v: %+v", *lsp, *ls, err)
	}

	return ops, lsp, podAnnotation, nil
}

func (oc *DefaultL3Controller) addLogicalPort(pod *kapi.Pod) (err error) {
	switchName := pod.Spec.NodeName
	...
	ops, lsp, podAnnotation, err = oc.controllerInfo.addLogicalPort(pod, oc.lsManager,
...
func (cInfo *controllerInfo) updatePodAnnotationWithRetry(origPod *kapi.Pod, podInfo *util.PodAnnotation, nadKeyName string) error {
	resultErr := retry.RetryOnConflict(util.OvnConflictBackoff, func() error {
		// Informer cache should not be mutated, so get a copy of the object
		pod, err := cInfo.watchFactory.GetPod(origPod.Namespace, origPod.Name)
		if err != nil {
			return err
		}

		cpod := pod.DeepCopy()
		cpod.Annotations, err = util.MarshalPodAnnotation(cpod.Annotations, podInfo, nadKeyName)
		if err != nil {
			return err
		}
		return cInfo.kube.UpdatePod(cpod)
	})
	if resultErr != nil {
...
func (cInfo *controllerInfo) delLSPOps(lsManager *lsm.LogicalSwitchManager, logicalPort, switchName,
	...
	ops, err := libovsdbops.DeleteLogicalSwitchPortsOps(cInfo.nbClient, nil, &lsw, &lsp)
	if err != nil {
