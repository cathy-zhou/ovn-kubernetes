...
func (oc *Controller) syncPods(pods []interface{}) {
		...
		for nadName := range networkMap {
			...
			if util.PodScheduled(pod) && util.PodWantsNetwork(pod) && err == nil {
				...
				if err = oc.waitForNodeLogicalSwitchInCache(switchName); err != nil {
					klog.Errorf("Failed to wait for switch %s to be added to cache. IP allocation may fail!",
						switchName)
				}
				if err = oc.lsManager.AllocateIPs(switchName, annotations.IPs); err != nil {
					klog.Errorf("Couldn't allocate IPs: %s for pod: %s on switch: %s"+
	...
	for _, n := range nodes {
		...
		if lsUUID, ok := oc.lsManager.GetUUID(switchName); !ok {
			klog.Errorf("Error getting logical switch for node %s network %s: %s", n.Name,
			...
			if err := oc.mc.nbClient.Get(ctx, ls); err != nil {
				klog.Errorf("Error getting logical switch for node %s network %s (UUID: %d) from ovn database (%v)",
					n.Name, oc.nadInfo.NetName, ls.UUID, err)
				continue
			}
...
func (oc *Controller) deleteLogicalPort(pod *kapi.Pod) {
	...
	for nadName, network := range networkMap {
		oc.delLogicalPort4Nad(pod, nadName, pod.Spec.NodeName, network)
	}
}
