...
func (nci *NetworkControllerInfo) syncMultiNetworkPolicies(multiPolicies []interface{}) error {
	...
	for _, npInterface := range multiPolicies {
		...
		if !nci.shouldApplyMultiPolicy(policy) {
			klog.V(5).Infof("[network (%s)] skipping syncing policy %s/%s",
				nci.GetNetworkName(), policy.Namespace, policy.Name)
			continue
		}
...
func (nci *NetworkControllerInfo) shouldApplyMultiPolicy(mpolicy *multinetworkpolicyapi.MultiNetworkPolicy) bool {
	...
	for _, networkName := range policyForNetworks {
		...
		if nci.IsNadExist(util.GetNadName(networkNamespace, networkName)) {
			return true
		}
	}
	return false
