...
// NewSecondaryL3Controller creates a new OVN controller for creating logical network
// infrastructure and policy for secondasry l3 network
func NewSecondaryL3Controller(existingOc Controller, cInfo *controllerInfo, netConfInfo util.NetConfInfo,
	nadKeyName string) (*SecondaryL3Controller, error) {
	var oc *SecondaryL3Controller

	if existingOc != nil {
		SecondaryL3Controller, ok := existingOc.(*SecondaryL3Controller)
		if !ok {
			// controller type different
			return nil, fmt.Errorf("network attachment definition %s does not share the same CNI config with network %s",
				nadKeyName, cInfo.NetName)
		}
		if !SecondaryL3Controller.L3NetConfInfo.Verify(netConfInfo) {
			return nil, fmt.Errorf("network attachment definition %s does not share the same CNI config with network %s",
				nadKeyName, cInfo.NetName)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("cluster subnet %s for network %s is invalid: %v",
				l3NetConfInfo.NetCidr, cInfo.NetName, err)
		}
	...
	oc.NetAttachDefs.Store(nadKeyName, true)
	return oc, nil
}
...
func (oc *SecondaryL3Controller) Stop() {
	oc.wg.Wait()
	close(oc.stopChan)
...
func (oc *SecondaryL3Controller) Run() error {
	...
	if err := oc.controllerInfo.updateL3TopologyVersion(); err != nil {
...
func (oc *SecondaryL3Controller) StartClusterMaster() error {
	...
	_, err := oc.controllerInfo.createOvnClusterRouter(false)
	return err
}
...
func (oc *SecondaryL3Controller) addLogicalPort(pod *kapi.Pod, nadKeyName string,
	...
	ops, lsp, podAnnotation, err = oc.controllerInfo.addLogicalPort(pod,
...
func (oc *SecondaryL3Controller) removePod(pod *kapi.Pod, portInfo *lpInfo) error {
	...
	pInfo, err := oc.controllerInfo.deleteLogicalPort(pod, portInfo, nadKeyName, nil, oc.lsManager, false)
	if err != nil {
		return err
	}
...
func (oc *SecondaryL3Controller) addUpdateNodeEvent(node *kapi.Node, nSyncs *nodeSyncs) error {
	...
	if nSyncs.syncClusterRouterPort {
		if err = oc.controllerInfo.syncNodeClusterRouterPort(node, hostSubnets); err != nil {
...
func (oc *SecondaryL3Controller) addNode(node *kapi.Node) ([]*net.IPNet, error) {
	updatedNodeAnnotation, hostSubnets, err := oc.controllerInfo.createNodeSubnetAnnotation(nil, node, oc.masterSubnetAllocator)
	if err != nil {
		return nil, err
	}
	...
	logicalSwitch, err := oc.controllerInfo.createNodeLogicalSwitch(node.Name, hostSubnets, "", false)
	if err != nil {
		return nil, err
	}

	// Add the node to the logical switch cache
	err = oc.lsManager.AddNode(logicalSwitch.Name, logicalSwitch.UUID, hostSubnets)
	if err != nil {
		return nil, err
	}
...
func (oc *SecondaryL3Controller) syncPods(pods []interface{}) error {
	...
	for _, podInterface := range pods {
		...
		err = oc.controllerInfo.updateExpectedLogicalPorts(pod, oc.lsManager, annotations, nadKeyName, expectedLogicalPorts)
		if err != nil {
			return err
		}
	}
...
func (oc *SecondaryL3Controller) syncNodes(nodes []interface{}) error {
	...
	for _, tmp := range nodes {
		...
		_ = oc.controllerInfo.updateExpectedNodes(node, oc.masterSubnetAllocator, foundNodes)
	}
