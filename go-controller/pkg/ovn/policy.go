...
// only needed for default network
func (nci *NetworkControllerInfo) updateStaleDefaultDenyACLNames(npType knet.PolicyType, gressSuffix string) error {
	if nci.IsSecondary() {
		return nil
	}
	cleanUpDefaultDeny := make(map[string][]*nbdb.ACL)
	p := func(item *nbdb.ACL) bool {
		if _, ok := item.ExternalIDs[types.NetworkNameExternalID]; ok {
			return false
		}
	...
	for namespace, aclList := range cleanUpDefaultDeny {
		...
		newACL := BuildACL(
			...
			aclList[0].Match,
			aclList[0].Action,
			nci.GetNamespaceACLLogging(namespace),
			aclT,
...
func (nci *NetworkControllerInfo) syncNetworkPolicies(networkPolicies []interface{}) error {
	...
	if err := nci.updateStaleDefaultDenyACLNames(knet.PolicyTypeEgress, egressDefaultDenySuffix); err != nil {
		return fmt.Errorf("cannot clean up egress default deny ACL name: %v", err)
	}
	if err := nci.updateStaleDefaultDenyACLNames(knet.PolicyTypeIngress, ingressDefaultDenySuffix); err != nil {
...
func addAllowACLFromNode(nodeName string, mgmtPortIP net.IP, nbClient libovsdbclient.Client) error {
	...
	nodeACL := BuildACL(getAllowFromNodeACLName(), types.DefaultAllowPriority, match,
		nbdb.ACLActionAllowRelated, nil, lportIngress, nil, (*util.NetNameInfo)(nil))
...
// cathy tbd hashName is the portGroupName without network prefix
func buildPortGroup(hashName, name string, ports []*nbdb.LogicalSwitchPort, acls []*nbdb.ACL, netInfo util.NetInfo) *nbdb.PortGroup {
	...
	if netInfo.IsSecondary() {
		...
		pgName = netInfo.GetPrefix() + hashName
	}
...
// TBD cathy pg is the portGroupName without network prefix
...
func (nci *NetworkControllerInfo) addPolicyToDefaultPortGroups(np *networkPolicy, aclLogging *ACLLoggingLevels) error {
	return nci.sharedNetpolPortGroups.DoWithLock(np.namespace, func(pgKey string) error {
		sharedPGs, loaded := nci.sharedNetpolPortGroups.LoadOrStore(pgKey, &defaultDenyPortGroups{
			ingressPortToPolicies: map[string]sets.String{},
			egressPortToPolicies:  map[string]sets.String{},
			policies:              map[string]bool{},
		})
		if !loaded {
			...
			if err != nil {
				nci.sharedNetpolPortGroups.Delete(pgKey)
...
func (nci *NetworkControllerInfo) delPolicyFromDefaultPortGroups(np *networkPolicy) error {
	return nci.sharedNetpolPortGroups.DoWithLock(np.namespace, func(pgKey string) error {
		sharedPGs, found := nci.sharedNetpolPortGroups.Load(pgKey)
		if !found {
			return nil
		}
...
func (nci *NetworkControllerInfo) deleteDefaultDenyPGAndACLs(namespace, policy string) error {
	...
	ops, err := libovsdbops.DeletePortGroupsOps(nci.nbClient, nil, nci.GetPrefix()+ingressPGName, nci.GetPrefix()+egressPGName)
	if err != nil {
		return err
	}
...
func (nci *NetworkControllerInfo) getNewLocalPolicyPorts(np *networkPolicy,
...
func (nci *NetworkControllerInfo) denyPGAddPorts(np *networkPolicy, portNamesToUUIDs map[string]string, ops []ovsdb.Operation) error {
	var err error
	...
	nci.sharedNetpolPortGroups.LockKey(pgKey)
	defer nci.sharedNetpolPortGroups.UnlockKey(pgKey)
	sharedPGs, ok := nci.sharedNetpolPortGroups.Load(pgKey)
	if !ok {
...
func (nci *NetworkControllerInfo) denyPGDeletePorts(np *networkPolicy, portNamesToUUIDs map[string]string, useLocalPods bool,
	...
	if len(portNamesToUUIDs) != 0 {
		...
		if !ok {
			...
			if len(ingressDenyPorts) != 0 || len(egressDenyPorts) != 0 {
				// db changes required
				ops, err = libovsdbops.DeletePortsFromPortGroupOps(nci.nbClient, ops, nci.GetPrefix()+ingressDenyPGName, ingressDenyPorts...)
				if err != nil {
					return fmt.Errorf("unable to get del ports from %s port group ops: %v", nci.GetPrefix()+ingressDenyPGName, err)
				}

				ops, err = libovsdbops.DeletePortsFromPortGroupOps(nci.nbClient, ops, nci.GetPrefix()+egressDenyPGName, egressDenyPorts...)
				if err != nil {
					return fmt.Errorf("unable to get del ports from %s port group ops: %v", nci.GetPrefix()+egressDenyPGName, err)
				}
			}
		}
	}
...
func (nci *NetworkControllerInfo) handleLocalPodSelectorAddFunc(np *networkPolicy, objs ...interface{}) error {
	np.RLock()
	defer np.RUnlock()
	if np.deleted {
		return nil
	...
	portNamesToUUIDs, policyPortUUIDs, errPods := nci.getNewLocalPolicyPorts(np, objs...)
	...
	if len(portNamesToUUIDs) > 0 {
		...
		if err = nci.denyPGAddPorts(np, portNamesToUUIDs, ops); err != nil {
...
func (nci *NetworkControllerInfo) handleLocalPodSelectorDelFunc(np *networkPolicy, objs ...interface{}) error {
	np.RLock()
	defer np.RUnlock()
	if np.deleted {
		return nil
	...
	portNamesToUUIDs, policyPortUUIDs, errPods := nci.getExistingLocalPolicyPorts(np, objs...)
	...
	if len(portNamesToUUIDs) > 0 {
		...
		if err = nci.denyPGDeletePorts(np, portNamesToUUIDs, false, ops); err != nil {
			return fmt.Errorf("unable to add new pod to default deny port group: %v", err)
		}
		// all operations were successful, update np.localPods
...
func (nci *NetworkControllerInfo) addLocalPodHandler(policy *knet.NetworkPolicy, np *networkPolicy) error {
	...
	syncFunc := func(objs []interface{}) error {
		...
		_ = nci.handleLocalPodSelectorAddFunc(np, objs...)
		return nil
	}
	retryLocalPods := nci.newRetryFrameworkWithParameters(
...
// that information.
func (nci *NetworkControllerInfo) createNetworkPolicy(policy *knet.NetworkPolicy, aclLogging *ACLLoggingLevels) (*networkPolicy, error) {
	...
	err := nci.networkPolicies.DoWithLock(npKey, func(npKey string) error {
		oldNP, found := nci.networkPolicies.Load(npKey)
		if found {
			// 1. Cleanup old policy if it failed to be created
			if cleanupErr := nci.cleanupNetworkPolicy(oldNP); cleanupErr != nil {
				return fmt.Errorf("cleanup for retrying network policy create failed: %v", cleanupErr)
			}
		}
		np, found = nci.networkPolicies.LoadOrStore(npKey, NewNetworkPolicy(policy))
		if found {
		...
		for i, ingressJSON := range policy.Spec.Ingress {
			...
			if hasAnyLabelSelector(ingressJSON.From) {
				...
				if err = ingress.ensurePeerAddressSet(nci.addressSetFactory); err != nil {
					return err
				}
			}
		...
		for i, egressJSON := range policy.Spec.Egress {
			...
			if hasAnyLabelSelector(egressJSON.To) {
				...
				if err = egress.ensurePeerAddressSet(nci.addressSetFactory); err != nil {
					return err
				}
			}
		...
		// 3. Add policy to default deny port group
		// Pods are not added to default deny port groups yet, this is just a preparation step
		err = nci.addPolicyToDefaultPortGroups(np, aclLogging)
		if err != nil {
			return err
		}
		...
		acls := nci.buildNetworkPolicyACLs(np, aclLogging)
...
func (nci *NetworkControllerInfo) addNetworkPolicy(policy *knet.NetworkPolicy) error {
	...
	// 1. save required namespace information to use for netpol create,
	npKey := getPolicyKey(policy)
	nsInfo, nsUnlock := nci.namespaceManager.getNamespaceLocked(policy.Namespace, true)
	if nsInfo == nil {
	...
	np, err = nci.createNetworkPolicy(policy, &aclLogging)
	defer func() {
		if err != nil {
			...
			cleanupErr := nci.networkPolicies.DoWithLock(npKey, func(npKey string) error {
				np, ok := nci.networkPolicies.Load(npKey)
				if !ok {
					klog.Infof("Deleting policy %s that is already deleted", npKey)
					return nil
				}
				return nci.cleanupNetworkPolicy(np)
			})
			if cleanupErr != nil {
	...
	// 3. lock namespace
	nsInfo, nsUnlock = nci.namespaceManager.getNamespaceLocked(policy.Namespace, false)
	if nsInfo == nil {
	...
	if nsInfo.aclLogging.Deny != aclLogging.Deny {
		if err := nci.updateACLLoggingForDefaultACLs(policy.Namespace, nsInfo); err != nil {
...
func (nci *NetworkControllerInfo) buildNetworkPolicyACLs(np *networkPolicy, aclLogging *ACLLoggingLevels) []*nbdb.ACL {
	acls := []*nbdb.ACL{}
...
func (nci *NetworkControllerInfo) deleteNetworkPolicy(policy *knet.NetworkPolicy) error {
	npKey := getPolicyKey(policy)
	...
	err := nci.networkPolicies.DoWithLock(npKey, func(npKey string) error {
		...
		if err := nci.cleanupNetworkPolicy(np); err != nil {
...
// that information.
func (nci *NetworkControllerInfo) cleanupNetworkPolicy(np *networkPolicy) error {
	...
	err = nci.denyPGDeletePorts(np, nil, true, ops)
	if err != nil {
	...
	err = nci.delPolicyFromDefaultPortGroups(np)
	if err != nil {
	...
	nci.networkPolicies.Delete(np.getKey())
	return nil
}
...
// ingress/egress address set
func (nci *NetworkControllerInfo) handlePeerPodSelectorDelete(np *networkPolicy, gp *gressPolicy, obj interface{}) error {
	np.RLock()
	defer np.RUnlock()
	if np.deleted {
		return nil
...
func (nci *NetworkControllerInfo) addPeerPodHandler(podSelector *metav1.LabelSelector,
	...
	// TBD cathy could to nci's retryFrame
	retryPeerPods := nci.newRetryFrameworkWithParameters(
...
func (nci *NetworkControllerInfo) handlePeerNamespaceAndPodAdd(np *networkPolicy, gp *gressPolicy,
...
func (nci *NetworkControllerInfo) handlePeerNamespaceAndPodDel(np *networkPolicy, gp *gressPolicy, obj interface{}) error {
	np.RLock()
	defer np.RUnlock()
	if np.deleted {
		return nil
	...
	if handler, ok := np.namespacedPodHandlers.Load(namespace.Name); ok {
		nci.watchFactory.RemovePodHandler(handler.(*factory.Handler))
		np.namespacedPodHandlers.Delete(namespace.Name)
	}

	pods, err := nci.watchFactory.GetPods(namespace.Name)
	if err != nil {
...
func (nci *NetworkControllerInfo) addPeerNamespaceAndPodHandler(namespaceSelector *metav1.LabelSelector,
	...
	retryPeerNamespaces := nci.newRetryFrameworkWithParameters(
...
func (nci *NetworkControllerInfo) handlePeerNamespaceSelectorAdd(np *networkPolicy, gp *gressPolicy, objs ...interface{}) error {
	...
	if updated {
		return nci.peerNamespaceUpdate(np, gp)
	}
	return nil
}

func (nci *NetworkControllerInfo) handlePeerNamespaceSelectorDel(np *networkPolicy, gp *gressPolicy, objs ...interface{}) error {
	np.RLock()
	if np.deleted {
		np.RUnlock()
		return nil
...
func (nci *NetworkControllerInfo) peerNamespaceUpdate(np *networkPolicy, gp *gressPolicy) error {
	...
	nsInfo, nsUnlock := nci.namespaceManager.getNamespaceLocked(np.namespace, true)
...
func (nci *NetworkControllerInfo) addPeerNamespaceHandler(
	namespaceSelector *metav1.LabelSelector,
	...
	syncFunc := func(objs []interface{}) error {
		...
		_ = nci.handlePeerNamespaceSelectorAdd(np, gress, objs...)
		return nil
	}
	retryPeerNamespaces := nci.newRetryFrameworkWithParameters(
