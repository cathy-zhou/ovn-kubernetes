...
func getMulticastDefaultExpectedData(fakeController *DefaultNetworkController, clusterPortGroup, clusterRtrPortGroup *nbdb.PortGroup) []libovsdb.TestData {
	...
	aclName = getACLName(aclIDs)
	egressMatch := fakeController.getACLMatch(types.ClusterRtrPortGroupName, match, aclEgress)
	...
	aclName = getACLName(aclIDs)
	ingressMatch := fakeController.getACLMatch(types.ClusterRtrPortGroupName, match, aclIngress)
...
func getMulticastPolicyExpectedData(fakeController *DefaultNetworkController, ns string, ports []string) []libovsdb.TestData {
	...
	egressMatch := fakeController.getACLMatch(pg_hash, fakeController.getMulticastACLEgrMatch(), aclEgress)
	...
	ingressMatch := fakeController.getACLMatch(pg_hash, mcastMatch, aclIngress)
...
var _ = ginkgo.Describe("OVN Multicast with IP Address Family", func() {
	...
	ginkgo.Context("on startup", func() {
		...
		ginkgo.It("cleans up Multicast resources when multicast is disabled", func() {
			app.Action = func(ctx *cli.Context) error {
				clusterPortGroup, clusterRtrPortGroup := getDefaultPortGroups()
				initialData := getMulticastDefaultExpectedData(fakeOvn.controller, clusterPortGroup, clusterRtrPortGroup)
				// test server doesn't delete de-referenced acls, so they will stay
				expectedData := initialData[:len(initialData)-2]
				clusterPortGroup, clusterRtrPortGroup = getDefaultPortGroups()

				nsData := getMulticastPolicyExpectedData(fakeOvn.controller, namespaceName1, nil)
		...
		ginkgo.It("updates stale namespace Multicast ACLs", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				clusterPortGroup, clusterRtrPortGroup := getDefaultPortGroups()
				expectedData := getMulticastDefaultExpectedData(fakeOvn.controller, clusterPortGroup, clusterRtrPortGroup)
				expectedData = append(expectedData, getMulticastPolicyStaleData(fakeOvn.controller, namespaceName1, nil)...)
				namespace1 := *newNamespace(namespaceName1)
				namespace1.Annotations[util.NsMulticastAnnotation] = "true"
				fakeOvn.startWithDBSetup(libovsdb.TestSetup{NBData: expectedData},
					&v1.NamespaceList{
		...
		ginkgo.It("cleans up namespace Multicast ACLs when namespace doesn't exist", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				clusterPortGroup, clusterRtrPortGroup := getDefaultPortGroups()
				initialData := getMulticastDefaultExpectedData(fakeOvn.controller, clusterPortGroup, clusterRtrPortGroup)
				initialData = append(initialData, getMulticastPolicyExpectedData(fakeOvn.controller, namespaceName1, nil)...)
