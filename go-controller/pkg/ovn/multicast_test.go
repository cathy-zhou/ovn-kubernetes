...
func (p multicastPolicy) getMulticastPolicyExpectedData(ns string, ports []string) []libovsdb.TestData {
	...
	egressMatch := getACLMatch(pg_hash, getMulticastACLEgrMatch(), lportEgressAfterLB, &util.DefaultNetInfo{})
	...
	ingressMatch := getACLMatch(pg_hash, mcastMatch, lportIngress, &util.DefaultNetInfo{})
