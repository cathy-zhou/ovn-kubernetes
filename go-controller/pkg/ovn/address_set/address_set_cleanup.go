...
func NonDualStackAddressSetCleanup(netInfo *util.NetInfo, nbClient libovsdbclient.Client) error {
	// For each address set, track if it is in old non dual stack
	// spec and in new dual stack spec
	const old = 0
	const new = 1
	if netInfo == nil {
		netInfo = &util.NetInfo{NetName: ovntypes.DefaultNetworkName, Prefix: "", IsSecondary: false, NadNames: nil}
	}
	addressSets := map[string][2]bool{}
	err := forEachAddressSet(*netInfo, nbClient, func(name string) error {
	...
	for name, spec := range addressSets {
		...
		if spec[old] {
			if spec[new] {
				...
				err := destroyAddressSet(*netInfo, nbClient, name)
				if err != nil {
					return err
				}
				continue
