...
func (bnc *BaseNetworkController) getMulticastPortGroupName(namespace string) string {
	return hashedPortGroup(bnc.getPortGroupName(namespace))
}
...
func (bnc *BaseNetworkController) createDefaultAllowMulticastPolicy() error {
	...
	acls := make([]*nbdb.ACL, 0, 2)
	for _, aclDir := range []aclDirection{aclEgress, aclIngress} {
		match := getACLMatch(bnc.getPortGroupName(types.ClusterRtrPortGroupName), mcastMatch, aclDir)
	...
	ops, err = libovsdbops.AddACLsToPortGroupOps(bnc.nbClient, ops, bnc.getPortGroupName(types.ClusterRtrPortGroupName), acls...)
	if err != nil {
		return err
	}
...
func (bnc *BaseNetworkController) disableMulticast() error {
	...
	err = libovsdbops.DeleteACLsFromPortGroups(bnc.nbClient, []string{bnc.getPortGroupName(types.ClusterRtrPortGroupName), bnc.getPortGroupName(types.ClusterPortGroupName)},
		mcastACLs...)
	if err != nil {
