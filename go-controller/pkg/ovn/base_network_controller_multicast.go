...
func (bnc *BaseNetworkController) createMulticastAllowPolicy(ns string, nsInfo *namespaceInfo) error {
	...
	aclDir := aclEgress
	egressMatch := bnc.getACLMatch(portGroupName, bnc.getMulticastACLEgrMatch(), aclDir)
	...
	aclDir = aclIngress
	ingressMatch := bnc.getACLMatch(portGroupName, bnc.getMulticastACLIgrMatch(nsInfo), aclDir)
	dbIDs = getNamespaceMcastACLDbIDs(ns, aclDir, bnc.controllerName)
	aclPipeline = aclDirectionToACLPipeline(aclDir)
...
func (bnc *BaseNetworkController) deleteMulticastAllowPolicy(nbClient libovsdbclient.Client, ns string) error {
	portGroupName := bnc.GetNetworkScopedName(getMulticastPortGroupName(ns))
...
func (bnc *BaseNetworkController) createDefaultDenyMulticastPolicy() error {
	...
	ops, err = libovsdbops.AddACLsToPortGroupOps(bnc.nbClient, ops, bnc.GetNetworkScopedName(types.ClusterPortGroupName), acls...)
	if err != nil {
		return err
	}
	...
	ops, err = libovsdbops.DeletePortGroupsOps(bnc.nbClient, ops, bnc.GetNetworkScopedName(legacyMulticastDefaultDenyPortGroup))
	if err != nil {
		return err
	}
...
func (bnc *BaseNetworkController) createDefaultAllowMulticastPolicy() error {
	...
	acls := make([]*nbdb.ACL, 0, 2)
	for _, aclDir := range []aclDirection{aclEgress, aclIngress} {
		match := bnc.getACLMatch(types.ClusterRtrPortGroupName, mcastMatch, aclDir)
	...
	ops, err = libovsdbops.AddACLsToPortGroupOps(bnc.nbClient, ops, bnc.GetNetworkScopedName(types.ClusterRtrPortGroupName), acls...)
	if err != nil {
		return err
	}
...
func (bnc *BaseNetworkController) podAddAllowMulticastPolicy(ns string, portInfo *lpInfo) error {
	return libovsdbops.AddPortsToPortGroup(bnc.nbClient, bnc.GetNetworkScopedName(getMulticastPortGroupName(ns)), portInfo.uuid)
}
...
func (bnc *BaseNetworkController) podDeleteAllowMulticastPolicy(ns string, portUUID string) error {
	return libovsdbops.DeletePortsFromPortGroup(bnc.nbClient, bnc.GetNetworkScopedName(getMulticastPortGroupName(ns)), portUUID)
}
