...
func (oc *Controller) assignEgressIPs(eIP *egressipv1.EgressIP) error {
	...
	if len(assignableNodes) == 0 {
		...
		oc.mc.recorder.Eventf(&eIPRef, kapi.EventTypeWarning, "NoMatchingNodeFound", "no assignable nodes for EgressIP: %s, please tag at least one node with label: %s", eIP.Name, util.GetNodeEgressLabel(oc.netconf.Name))
...
func (oc *Controller) initEgressIPAllocator(node *kapi.Node) (err error) {
	...
	if _, exists := oc.eIPC.allocator[node.Name]; !exists {
		...
		v4IfAddr, v6IfAddr, err := util.ParseNodePrimaryIfAddr(node, oc.netconf.Name)
		if err != nil {
