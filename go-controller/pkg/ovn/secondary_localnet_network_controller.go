...
func (oc *SecondaryLocalnetNetworkController) Cleanup(netName string) error {
	klog.Infof("Delete OVN logical entities for %s network controller of network %s", types.LocalnetTopology, netName)
...
func (oc *SecondaryLocalnetNetworkController) Init() error {
	...
	for _, excludeSubnet := range localnetNetConfInfo.ExcludeSubnets {
