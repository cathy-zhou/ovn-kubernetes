...
type PodRecorder struct {
	...
	records map[string]map[kapimtypes.UID]*record
...
func (pr *PodRecorder) Run(sbClient libovsdbclient.Client, stop <-chan struct{}) {
	...
	pr.records = map[string]map[kapimtypes.UID]*record{}
...
func (pr *PodRecorder) processItem(i item) {
	...
	case addPod:
		r := &record{firstSeenTS: i.timestamp, podTimeInfo: map[string]timeInfo{}}
		rMap, ok := pr.records[i.netNameInfo.NetName]
		if !ok {
			pr.records[i.netNameInfo.NetName] = map[kapimtypes.UID]*record{i.uid: r}
		} else {
			rMap[i.uid] = r
		}
	case cleanPod:
		...
		if len(rMap) == 0 {
			delete(pr.records, i.netNameInfo.NetName)
		}
