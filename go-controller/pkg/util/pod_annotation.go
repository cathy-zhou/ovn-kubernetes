...
func GetPodIPsOfNetwork(pod *v1.Pod, nInfo NetInfo) ([]net.IP, error) {
	...
	if !nInfo.IsSecondary() {
		...
		on, networkMap, err = PodWantsMultiNetwork(pod, nInfo)
		if err != nil {
			return nil, err
		} else if !on {
...
func GetK8sPodDefaultNetwork(pod *v1.Pod) (*nadapi.NetworkSelectionElement, error) {
	var netAnnot string
...
func GetK8sPodAllNetworks(pod *v1.Pod) ([]*nadapi.NetworkSelectionElement, error) {
