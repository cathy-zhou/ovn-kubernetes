...
func newCommonNodeNetworkControllerInfoCommon(kubeClient clientset.Interface, kube kube.Interface,
...
func NewCommonNodeNetworkControllerInfo(kubeClient clientset.Interface, wf factory.NodeWatchFactory,
	...
	return newCommonNodeNetworkControllerInfoCommon(kubeClient, &kube.Kube{KClient: kubeClient}, wf, eventRecorder, name, isOvnUpEnabled)
}
...
func newDefaultNodeNetworkControllerCommon(cnnci *CommonNodeNetworkControllerInfo, stopChan chan struct{},
...
func NewDefaultNodeNetworkController(cnnci *CommonNodeNetworkControllerInfo) *DefaultNodeNetworkController {
	...
	wg := &sync.WaitGroup{}
	n := newDefaultNodeNetworkControllerCommon(cnnci, stopChan, wg)
...
func (nc *DefaultNodeNetworkController) Start(ctx context.Context) error {
	...
	if config.OvnKubeNode.Mode != types.NodeModeDPU {
		...
		cniServer, err = cni.NewCNIServer("", nc.atomicOvnUpEnabled == 1, nc.watchFactory, kclient.KClient)
		if err != nil {
			return err
		}
	}
		...
		if initialTopoVersion != types.OvnCurrentTopologyVersion {
			...
			go func() {
				...
				if initialTopoVersion < types.OvnPortBindingTopoVersion && cniServer != nil &&
					nc.atomicOvnUpEnabled == 0 && !config.OvnKubeNode.DisableOVNIfaceIdVer {
