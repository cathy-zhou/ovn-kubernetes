...
"gateway_ip":"192.168.2.1"}}`,
		...
		It("Creates PodInterfaceInfo in NodeModeFull mode", func() {
			...
			pif, err := PodAnnotation2PodInfo(podAnnot, false, podUID, "", ovntypes.DefaultNetworkName, (*util.NetNameInfo)(nil), config.Default.MTU)
			Expect(err).ToNot(HaveOccurred())
		...
		It("Creates PodInterfaceInfo with checkExtIDs true", func() {
			pif, err := PodAnnotation2PodInfo(podAnnot, true, podUID, "", ovntypes.DefaultNetworkName, (*util.NetNameInfo)(nil), config.Default.MTU)
			Expect(err).ToNot(HaveOccurred())
