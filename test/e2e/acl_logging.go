...
var _ = Describe("ACL Logging for NetworkPolicy", func() {
	...
	It("the logs have the expected log level", func() {
		...
		composedPolicyNameRegex := fmt.Sprintf("NP:%s:%s", sharedPortGroup, egressDefaultDenySuffix)
...
func getSharedPortGroupName(policy *knet.NetworkPolicy) string {
	podSelectorString := shortLabelSelectorString(&policy.Spec.PodSelector)
	readableGroupName := fmt.Sprintf("%s_%s", policy.Namespace, podSelectorString)
	return HashForOVN(readableGroupName)
}
